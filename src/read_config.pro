...
COMMON GLOBAL_VARS, MISSING, MAX_PBL_HT
  ...
  if ds.debug gt 0 then print, 'The config path is ', conf_path
  ...
  ;if ds.debug gt 0 then print, inv_strength_thres
  parts = strsplit(buff[1], ',', /extract)
  instab_threshold  = float(parts[1])
  ;if ds.debug gt 0 then print, instab_threshold
  parts = strsplit(buff[2], ',', /extract)
  overshoot_threshold  = float(parts[1])
  ;if ds.debug gt 0 then print, overshoot_threshold
