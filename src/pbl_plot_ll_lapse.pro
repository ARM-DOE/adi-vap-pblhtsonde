PRO PBL_PLOT_LL_LAPSE, ss_theta, ss_height, smoothed_lapse_ss, level1, level2, pbl_ll, regime_type, date, pngpath, vapname
...
COMMON GLOBAL_VARS, MISSING, MAX_PBL_HT
  ...
  ;axis, xaxis=1, color=d_red, xtitle = 'Windspeed (m/s)' ;+ date
   plot,/noerase,smoothed_lapse_ss,ss_height-alt_var0,psym=2,color=d_red,xrange=[-.04,.04],xstyle=4,$
    yrange=[0,MAX_PBL_HT],ystyle=4
  axis, xaxis=1, color=d_red, xtitle = 'Potential temperature lapse rate (K/m) ' ;+ date

  oplot, [-.04,0.04],[level1-alt_var0, level1-alt_var0], color=d_blue
  oplot, [-.04,0.04],[level2-alt_var0, level2-alt_var0], color=d_black
  oplot, [-.04,0.04],[pbl_ll-alt_var0, pbl_ll-alt_var0], color=d_green, linestyle=4
  xyouts, -.035, 3800, 'PBL = ' + strtrim(string(format='(I5.2)',pbl_ll-alt_var0)) + ' m AGL', color=d_green 
  xyouts, -.035, 3600, level1lbl, color=d_blue 
  xyouts, -.035, 3400, level2lbl, color=d_black 
  xyouts,-.01, -400, 'Created on '+systime(0), color=d_black
  xyouts,-.01,-550, vapname + ' plot  for ' + date,color=d_black
  ;;Title
  xyouts,-.035, 5300, 'PBL using Liu and Liang method for regime type ' + regime_type + ' for '  + date, color=d_black
